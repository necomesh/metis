...
`POST /api/v1/ai/sessions/:sid/messages` SHALL accept user message content, store it, and trigger agent execution. The response SHALL be the stored message. The actual agent response is delivered via SSE stream.
...
The system SHALL provide `GET /api/v1/ai/sessions/:sid/stream` as an SSE endpoint. Events SHALL use the unified event protocol (llm_start, content_delta, tool_call, tool_result, plan, step_start, done, cancelled, error). Each event SHALL carry an incrementing event ID for reconnection support.

#### Scenario: SSE connection
- **WHEN** user opens SSE connection to `/api/v1/ai/sessions/:sid/stream`
- **THEN** system SHALL stream events from the current or next execution in real-time

#### Scenario: SSE reconnection
- **WHEN** SSE connection drops and client reconnects with Last-Event-ID header
- **THEN** system SHALL replay events from the given ID onward

#### Scenario: No active execution
- **WHEN** user opens SSE and no execution is running
- **THEN** system SHALL keep the connection open and wait for the next execution
