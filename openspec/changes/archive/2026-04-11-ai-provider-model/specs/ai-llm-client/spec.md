...
The system SHALL support streaming chat completion that returns events via a Go channel.

#### Scenario: Stream chat request
- **WHEN** caller invokes `ChatStream(ctx, ChatRequest)`
- **THEN** the client returns a `<-chan StreamEvent` that emits content deltas, tool calls, and a final done event with usage stats
