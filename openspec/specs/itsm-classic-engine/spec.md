...
ClassicEngine.Start() SHALL 解析 `ServiceDefinition.workflow_json`，找到 `start` 节点，沿其唯一出边找到第一个业务节点，创建对应的 TicketActivity（status=pending 或 in_progress），并将工单状态设为 `in_progress`。工单 SHALL 在创建时保存 workflow_json 的快照副本。

#### Scenario: 正常启动经典流程
- **WHEN** 用户创建一个 engine_type="classic" 的工单，且服务的 workflow_json 合法
- **THEN** 系统创建工单（status=in_progress），保存 workflow_json 快照到工单记录，创建 start 节点出边目标的 TicketActivity，记录 Timeline 事件"流程启动"
...
- **WHEN** start 节点的出边目标是 gateway / action / notify 等自动节点
- **THEN** ClassicEngine.Start() 自动递归处理这些自动节点，直到到达需要人工干预的节点（form/approve/process/wait）或 end 节点

---

### Requirement: ClassicEngine 图遍历 — Progress
ClassicEngine.Progress() SHALL 接收当前 Activity 和 outcome，在 workflow_json 中找到当前节点的出边，匹配 outcome 对应的边，创建目标节点的 Activity。自动节点（gateway/action/notify）SHALL 立即递归处理，人工节点（form/approve/process/wait）创建 pending Activity 后停止。到达 end 节点时 SHALL 将工单状态设为 `completed`。

#### Scenario: 人工节点正常流转
- **WHEN** 处理人对一个 approve 节点的 Activity 提交 outcome="approved"
- **THEN** 系统将当前 Activity 标记为 completed，找到 outcome="approved" 对应的出边，创建目标节点的 Activity，记录 Timeline 事件

#### Scenario: 流转到达 end 节点
- **WHEN** Progress 的目标节点是 end 类型
- **THEN** 系统创建 end 节点的 Activity（status=completed），将工单状态设为 `completed`，记录 Timeline 事件"流程完结"
...
#### Scenario: 自动步进深度限制
- **WHEN** Progress 过程中自动节点（gateway→gateway→...）递归超过 50 层
- **THEN** 系统中止执行，将工单标记为异常状态，记录 Timeline 错误事件"流程自动步进超过最大深度"

---

### Requirement: ClassicEngine 图遍历 — Cancel
ClassicEngine.Cancel() SHALL 将工单状态设为 `cancelled`，将所有活跃的 Activity（status=pending 或 in_progress）标记为 `cancelled`，记录取消原因到 Timeline。

#### Scenario: 取消正在执行的工单
- **WHEN** 管理员取消一个 in_progress 的工单
- **THEN** 所有活跃 Activity 状态设为 cancelled，工单状态设为 cancelled，Timeline 记录"工单取消：{reason}"
...
gateway 节点 SHALL 根据条件自动选择出边。gateway 是自动节点，不创建需要人工干预的 Activity。节点 `data.conditions` 定义条件列表，每个条件关联一条出边。条件评估基于工单字段值（Ticket 字段、最近 Activity 的 result 字段、表单数据）。

#### Scenario: 条件匹配到对应出边
- **WHEN** 流程到达 gateway 节点，第一个条件评估为 true
- **THEN** 系统沿该条件对应的出边继续，跳过后续条件评估

#### Scenario: 无条件匹配时走默认边
- **WHEN** gateway 节点的所有条件均评估为 false，但存在 default=true 的出边
- **THEN** 系统沿默认出边继续

#### Scenario: 无条件匹配且无默认边
- **WHEN** gateway 节点所有条件均为 false，且没有默认出边
...
end 节点 SHALL 标记流程完成。到达 end 节点时，系统 SHALL 将工单状态设为 `completed`，记录流程完结 Timeline 事件。一个合法的 workflow_json 中 SHALL 至少有一个 end 节点。end 节点 SHALL 无出边。

#### Scenario: 正常到达 end 节点
- **WHEN** 流程流转到达 end 节点
- **THEN** 工单状态设为 completed，记录 Timeline "流程完结"

#### Scenario: 多个 end 节点（不同分支）
- **WHEN** workflow_json 包含多个 end 节点（如正常完结和异常完结分支各一个）
- **THEN** 到达任一 end 节点均完结工单，end 节点的 `data.label` 记录到 Timeline 区分完结类型

---

### Requirement: Workflow JSON Schema 校验
系统 SHALL 在保存 workflow_json 时进行完整性校验。校验规则包括：有且仅有一个 start 节点；至少一个 end 节点；start 节点有且仅有一条出边；end 节点无出边；所有边的 source 和 target 引用存在的节点 ID；无孤立节点（每个非 start 节点至少有一条入边）；gateway 节点的每条非默认出边 SHALL 配置条件；gateway 节点至少有两条出边；节点类型 SHALL 是合法的 9 种之一。
...
- **WHEN** workflow_json 中没有 type="start" 的节点
- **THEN** 校验失败，返回错误"工作流必须包含一个开始节点"

#### Scenario: 多个 start 节点
- **WHEN** workflow_json 中有两个 type="start" 的节点
- **THEN** 校验失败，返回错误"工作流只能包含一个开始节点"

#### Scenario: 缺少 end 节点
- **WHEN** workflow_json 中没有 type="end" 的节点
- **THEN** 校验失败，返回错误"工作流必须包含至少一个结束节点"

#### Scenario: 边引用不存在的节点
- **WHEN** 某条边的 source 或 target ID 在节点列表中不存在
- **THEN** 校验失败，返回错误"边 {edge_id} 引用了不存在的节点 {node_id}"

#### Scenario: 孤立节点
- **WHEN** 某个非 start 节点没有任何入边
- **THEN** 校验失败，返回错误"节点 {node_id} 没有入边，无法到达"

#### Scenario: gateway 出边缺少条件
- **WHEN** gateway 节点的某条非默认出边没有配置 condition
- **THEN** 校验失败，返回错误"网关节点 {node_id} 的出边 {edge_id} 缺少条件配置"

#### Scenario: gateway 出边不足
- **WHEN** gateway 节点只有一条出边
- **THEN** 校验失败，返回错误"网关节点 {node_id} 至少需要两条出边"

#### Scenario: 非法节点类型
- **WHEN** 节点的 type 不在 9 种合法类型中
