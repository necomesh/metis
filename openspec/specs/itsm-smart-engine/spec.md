...
#### Scenario: 使用知识库补充上下文
- **WHEN** 服务定义的 `knowledge_base_ids` 不为空且关联知识库有已编译的知识节点
- **THEN** 系统 SHALL 基于工单 summary 从关联知识库中检索相关知识节点（通过 KnowledgeService），将检索结果作为补充上下文注入 system message
...
服务定义的 `knowledge_base_ids` 关联的知识库 SHALL 在 Agent 决策时作为补充上下文注入。

#### Scenario: 知识检索注入
- **WHEN** 服务定义关联了知识库且知识库有已编译的知识节点
- **THEN** 系统 SHALL 基于工单 summary 检索相关知识节点（通过 KnowledgeService），将检索结果格式化后作为 system prompt 的补充部分：`## 相关知识\n\n{knowledge_nodes}`
...
#### Scenario: 知识库绑定
- **WHEN** 管理员编辑智能服务
- **THEN** 系统 SHALL 提供知识库多选组件（从 AI App 获取知识库列表），选择后保存 `knowledge_base_ids` 到 ServiceDefinition
