...
test("服务台聊天按 AI SDK UI Message Stream 增量渲染", async ({ page }) => {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (url.pathname === "/api/v1/install/status") {
      await route.fulfill({ json: ok({ installed: true }) })
      return
    }
    ...
    if (url.pathname === "/api/v1/notifications/unread-count") {
      await route.fulfill({ json: ok({ count: 0 }) })
      return
    }
  ...
  try {
    ...
    await page.getByPlaceholder("描述你的 IT 诉求...").fill("我想申请 VPN，线上支持用\n\nadmin\npassword")
    await page.keyboard.press("Enter")

    await expect(page.getByText("正在")).toBeVisible()
    await expect(page.getByText("正在为你准备 VPN 申请草稿")).toHaveCount(0, { timeout: 80 })

    const toolActivity = page.getByTestId("chat-tool-activity")
    ...
    await expect(page.getByTestId("itsm-draft-form-surface")).toBeVisible()
    await expect(page.getByText("VPN 账号")).toBeVisible()
    await expect(page.getByText("正在为你准备 VPN 申请草稿")).toHaveCount(0)
  } finally {
