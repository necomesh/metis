...
func (r *ProcessDefRepo) Create(pd *ProcessDef) error {
	return r.db.Create(pd).Error
}

func (r *ProcessDefRepo) FindByID(id uint) (*ProcessDef, error) {
	var pd ProcessDef
...
func (r *ProcessDefRepo) FindByName(name string) (*ProcessDef, error) {
	var pd ProcessDef
...
func (r *ProcessDefRepo) List(params ProcessDefListParams) ([]ProcessDef, int64, error) {
	if params.Page < 1 {
		params.Page = 1
	}
...
func (r *ProcessDefRepo) Update(id uint, updates map[string]any) error {
	return r.db.Model(&ProcessDef{}).Where("id = ?", id).Updates(updates).Error
}

func (r *ProcessDefRepo) Delete(id uint) error {
	return r.db.Delete(&ProcessDef{}, id).Error
}
