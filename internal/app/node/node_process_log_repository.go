...
func (r *NodeProcessLogRepo) CreateBatch(logs []NodeProcessLog) error {
	if len(logs) == 0 {
		return nil
	}
...
func (r *NodeProcessLogRepo) DeleteBefore(cutoff time.Time) (int64, error) {
	result := r.db.Where("timestamp < ?", cutoff).Delete(&NodeProcessLog{})
