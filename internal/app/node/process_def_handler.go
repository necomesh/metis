...
func (h *ProcessDefHandler) Create(c *gin.Context) {
	...
	pd := &ProcessDef{
		...
		Env:            JSONMap(req.Env),
		ConfigFiles:    JSONArray(req.ConfigFiles),
		ProbeType:      req.ProbeType,
		ProbeConfig:    JSONMap(req.ProbeConfig),
		RestartPolicy:  req.RestartPolicy,
		MaxRestarts:    req.MaxRestarts,
		ResourceLimits: JSONMap(req.ResourceLimits),
	}
	if pd.ProbeType == "" {
		pd.ProbeType = ProbeTypeNone
	}
	if pd.RestartPolicy == "" {
		pd.RestartPolicy = RestartPolicyAlways
	}
	if pd.MaxRestarts == 0 {
		pd.MaxRestarts = 10
	}
...
func (h *ProcessDefHandler) List(c *gin.Context) {
	...
	result := make([]ProcessDefResponse, len(items))
...
func (h *ProcessDefHandler) Update(c *gin.Context) {
	...
	if req.Env != nil {
		updates["env"] = JSONMap(*req.Env)
	}
	if req.ConfigFiles != nil {
		updates["config_files"] = JSONArray(*req.ConfigFiles)
	}
	if req.ProbeType != nil {
		updates["probe_type"] = *req.ProbeType
	}
	if req.ProbeConfig != nil {
		updates["probe_config"] = JSONMap(*req.ProbeConfig)
	}
	...
	if req.ResourceLimits != nil {
		updates["resource_limits"] = JSONMap(*req.ResourceLimits)
	}
